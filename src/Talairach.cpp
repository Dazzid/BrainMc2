...
//  BrainMc2
//
//  Created by David Dalmazzo on 21/03/16.
//
//
...
//------------------------------------------------------------------------------
void Talairach::setup(string _filepath){
    filepath=_filepath;
    ...
    if(existsFile(filepath)){ // TODO This only will work for OSX systems
        ...
        cout << "Talairach Error :: talairach.jar does not exist in assets/ folder" << endl;
    }
}

//------------------------------------------------------------------------------
void Talairach::close(){
    //a check for a jar File Exitance
    if(bConnected){
        commandline = "java -cp "+ filepath +" org.talairach.AtlasServer 1600";
        system(commandline.c_str());
...
//--------------------------------------------------------------
void Talairach::connectT(){
    //a check for a jar File Exitance
    if(existsFile("../../../assets/talairach.jar")){ // TODO This only will work for OSX systems
        commandline = "java -cp ../../../assets/talairach.jar org.talairach.AtlasServer 1600";
        system(commandline.c_str());
        bConnected = true; //TODO better to read buffer output to check connection message
    }else {
        cout << "Talairach Error :: talairach.jar does not exist in bin/data folder" << endl;
    }
    
}

//------------------------------------------------------------------------------
void Talairach::drawDebug(int x, int y){
    ...
    //Info
    //gl::drawString("Press '1','2', or '3' to get a sample with diferent data from The Talairach Client", Vec2i(x, y));
    //gl::drawString("Move mouse to change Brain Point location x="+toString(locBrain.x)+" y="+toString(locBrain.y)+" z="+toString(locBrain.z), Vec2i(x, y+MARGINTEXT));
    //gl::drawString("Press '+' or '-' for change the cubeSize request", Vec2i(x, y+MARGINTEXT*2));
    
    gl::drawString("Last point requested ", Vec2i(x, y+MARGINTEXT*3));
    gl::drawString(toString(point3d), Vec2i(x, y+MARGINTEXT*4));
    
    //DaerProbability map
    gl::drawString("Probability Map: ", Vec2i(x, y+yposProb+ MARGINTEXT));
    ...
    if(actualBrainArea>0){
        gl::drawString(toString(myBrainAreas[actualBrainArea]), Vec2i(x, y+yposProb+ MARGINTEXT*2) );
        gl::drawString(toString(probMap), Vec2i(x, y+yposProb+ MARGINTEXT*3) );
    }
    
    //Labels
    gl::drawString("Labels: ", Vec2i(x, y+yposLabel));
    gl::color(10, 10, 10);
    drawRequestedLabels(vectorLabels, x,y+yposLabel);
    
    gl::drawString("Labels with a cubeSize=: " + toString(cubeSize), Vec2i(x, y+yposLabelCube));
    gl::color(10, 10, 10);
...
//--------------------------------------------------------------
void Talairach::drawRequestedLabels(vector<string> vlabels, int x, int y){
    
    for (int i=0; i<vlabels.size(); i++) {
        gl::drawString(vlabels[i], Vec2i( x, i*MARGINTEXT + y + MARGINTEXT*2));
    }
...
//GETTERS
//--------------------------------------------------------------
vector<string> Talairach::get(Vec3f pos){
    ...
    string slabelop;
    if (isearchOption==3){
        slabelop = " "+toString(isearchOption)+":" + " "+toString(cubeSize)+",";
    }else{
        slabelop = " "+toString(isearchOption)+",";
    }
    commandline += slabelop;
    string cubePointStr =	  " " +toString(pos.x)
    + ", "+toString(pos.y)
    + ", "+toString(pos.z);
...
//--------------------------------------------------------------
vector<string> Talairach::request(string commandline){
...
//--------------------------------------------------------------
float Talairach::getStructuralProbMap(Vec3f pos){
    if (localhost) {
        string slocalhost = " host=localhost:1600";
        commandline = "java -cp ../../../assets/talairach.jar org.talairach.PointToTD";
        string slabelop = " 1,";
        commandline += slocalhost;
        commandline += slabelop;
        
    }else commandline = "java -cp ../../../assets/talairach.jar org.talairach.PointToTD 1,";
    //commandline += " 15, 10, 12";
    
    string cubePointStr = " "+toString(pos.x)+", "+toString(pos.y)+", "+toString(pos.z);
    commandline += cubePointStr;
...
//--------------------------------------------------------------
vector<string> Talairach::getLabels(Vec3f pos){
    ...
    if (localhost) {
        ...
        commandline = "java -cp ../../../assets/talairach.jar org.talairach.PointToTD 2,";
    }
    string cubePointStr = " "+toString(pos.x)+", "+toString(pos.y)+", "+toString(pos.z);
    commandline += cubePointStr;
...
//--------------------------------------------------------------
vector<string> Talairach::getLabelsArroundCube(Vec3f pos, int _cubeSize){
    //Save last data used to request
    point3d = pos;
    cubeSize = _cubeSize;
    ...
    }else commandline = "java -cp ../../../assets/talairach.jar org.talairach.PointToTD 3:";
    string cubeSizeStr = " "+toString(cubeSize)+",";
...
//--------------------------------------------------------------
void Talairach::requestTL(vector<string> &vlabel, string commandline){
...
//--------------------------------------------------------------
string Talairach::readBuffer4Label(char _buffer[]){
    
    string resultsubbuff = "";
    
    if ( sizeof(_buffer) > 0 ){
        resultsubbuff = toString(_buffer);
    }
...
//--------------------------------------------------------------
float Talairach::requestSPM(string commandline){
...
//--------------------------------------------------------------
float  Talairach::readBufferSPM(char _buffer[]){
    
    bool bProbMap = false;
    int numberofchars = 0;
    
    //Vec3f location = Vec3f(-1,-1,-1);
    float myresult = -1;
    ...
    if(bProbMap){
        ...
        myresult = std::stof(resultsubbuff);
    }
...
//--------------------------------------------------------------
void Talairach::setLocalhost(bool _host){
    localhost = _host;
    if (localhost==true){
        slocalhost = " host=localhost:1600";
    }else{
...
//--------------------------------------------------------------
void Talairach::setSearchOption(int _searchOption){
...
void Talairach::setSizeCube(int _sizecube){
    //Sizes of 3, 5, 7, 9 and 11 are accepted.
    if(_sizecube == 3 || _sizecube == 5 || _sizecube == 7 || _sizecube == 9 || _sizecube == 11){
        cubeSize = _sizecube;
    }
    else {
        cout << "Talairach:: Set a cubeSize error. Only 3, 5, 7, 9 and 11 are accepted" << endl;
    }
}
